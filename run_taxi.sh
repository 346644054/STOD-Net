...
#SBATCH -J taxi_layer
#SBATCH -o taxi_layer.%J.out
#SBATCH -e taxi_layer.%J.err
#SBATCH --array=1-70
...
loss_values=( 'l1' )
depth_values=( 16 28 40 52 64 76 88 )
lr_values=(  1e-4  )
step_values=( 1 )
gcn_values=( 'gat' )
file_values=( 'taxi' )
...
python main.py -traffic ${file} -gcn_type ${gcn} -lr ${lr} -depth ${depth} -loss ${loss} -epoch_size 500 -ibex -close_size ${c} -period_size ${p} -trend_size ${t} -norm_type ${norm} -${fusion} -${transfer} -gcn_layer ${gcn_layer} -gate_type ${gate} -beta ${beta} -alpha ${alpha} -exp ${exp} -batch_size 128