...
python main.py -traffic ${file} -gcn_type ${gcn} -lr ${lr} -depth ${depth} -loss ${loss} -epoch_size 100 -ibex -close_size ${c} -period_size ${p} -trend_size ${t} -norm_type ${norm} -${fusion} -${transfer} -gcn_layer ${gcn_layer} -gate_type ${gate} -beta ${beta} -alpha ${alpha} -exp ${exp} -batch_size 128